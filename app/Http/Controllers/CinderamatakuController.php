...
class CinderamatakuController extends Controller
{
     public function index()
    {
        ...
        $data = array();
        foreach ($cinderamata as $coba) {
            array_push($data, array(
                'id_cinderamata' => $coba->id_cinderamata,
                'nama_toko' => $coba->nama_toko,
                'alamat' => $coba->alamat,
                'jenis_cinderamata'=>$coba->jenis_cinderamata,
                'deskripsi' => $coba->deskripsi,
                'gambar' => $coba->gambar,
                'post' => Post::where("name", $coba->nama_toko)->first()
            ));
        }
         ...
         if(auth()->user()){
            if(auth()->user()->role == "user"){
                ...
                if ($result_userExist!=null){
                    ...
                    return view ('cinderamataku')->with(['data' => $data, 'cinderamata' => $cinderamata, "rekomendasis" => array()]);
                }
                ...
                foreach ($final as $destinasi) {
                    ...
                    // }
                    if($post->jenis == "toko"){
                        $toko2 = Cinderamata::where("nama_toko", "=",$post->name)->first();
                        $item = [
                            "nama" => $toko2->nama_toko,
                            "alamat" => $toko2->alamat,
                            "prediksi_bintang" => $destinasi['hasil'],
                ...
                return view ('cinderamataku')->with(['data' => $data, 'cinderamata' => $cinderamata, "rekomendasis" => $rekomendasi]);

            }else{
                return view ('cinderamataku')->with(['data' => $data, 'cinderamata' => $cinderamata]);
            }
    ...
    public function show(Cinderamata $cinderamata, Post $post, Forum $forum)
    {
         $post = Post::where("name", $cinderamata->nama_toko)->first();
         $forum= Forum::where("id", $cinderamata->id_cinderamata)->where("jenis",'cinderamata')->orderBy('created_at','desc')->paginate(10);
        return view('cinderamata1',compact('cinderamata','forum'),compact('post'));
    }

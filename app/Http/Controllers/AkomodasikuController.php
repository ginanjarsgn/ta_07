   ...
   public function index()
    {
        ...
        $akomodasi=\App\Models\Akomodasi::all();
         $data = array();
        foreach ($akomodasi as $coba) {
            array_push($data, array(
                'id_akomodasi' => $coba->id_akomodasi,
                'nama_akomodasi' => $coba->nama_akomodasi,
                'alamat' => $coba->alamat,
                'fasilitas'=>$coba->fasilitas,
                'deskripsi' => $coba->deskripsi,
                'gambar' => $coba->gambar,
                'post' => Post::where("name", $coba->nama_akomodasi)->first()
            ));
        }
        ...
        if(auth()->user()){
            if(auth()->user()->role == "user"){
                ...
                if ($result_userExist!=null){
                    ...
                    return view ('akomodasiku')->with(['data' => $data, 'akomodasi' => $akomodasi, "rekomendasis" => array()]);
                }
                ...
                foreach ($final as $destinasi) {
                    ...
                    //     array_push($rekomendasi, $item);
                    // }else 
                    if($post->jenis == "akomodasi"){
                        $akomodasi2 = Akomodasi::where("nama_akomodasi", "=",$post->name)->first();
                        $item = [
                            "nama" => $akomodasi2->nama_akomodasi,
                            "alamat" => $akomodasi2->alamat,
                            "prediksi_bintang" => $destinasi['hasil'],
                            "url" => "akomodasiku1/" . $akomodasi2->id_akomodasi,
                ...
                return view ('akomodasiku')->with(['data' => $data, 'akomodasi' => $akomodasi, "rekomendasis" => $rekomendasi]);

            }else{
                return view ('akomodasiku')->with(['data' => $data, 'akomodasi' => $akomodasi]);
            }
    ...
    public function show(Akomodasi $akomodasi, Post $post, Forum $forum)
    {   
        $post = Post::where("name", $akomodasi->nama_akomodasi)->first();
        $forum= Forum::where("id", $akomodasi->id_akomodasi)->where("jenis",'akomodasi')->orderBy('created_at','desc')->paginate(10);
        return view('akomodasiku1',compact('akomodasi','forum'),compact('post'));
    }
}
